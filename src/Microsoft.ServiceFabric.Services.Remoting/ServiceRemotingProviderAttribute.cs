...
namespace Microsoft.ServiceFabric.Services.Remoting
{
    ...
    ///         <see
    ///             cref="ServiceRemotingExtensions.CreateServiceRemotingListener{TStatefulService}(TStatefulService,System.Fabric.StatefulServiceContext)" />
    ///         and
    ///         <see
    ///             cref="ServiceRemotingExtensions.CreateServiceRemotingListener{TStatelessService}(TStatelessService,System.Fabric.StatelessServiceContext)" />
    ///         methods on the runtime to create a default <see cref="IServiceRemotingListener" /> for the stateful and
    ///         stateless services.
    ///     </para>
    ///     <para>
    ///         On client side, implementation of this attribute is looked up by
    ///         <see cref="V1.Client.ServiceProxyFactory" /> constructor to create a default
    ///         <see cref="V1.Client.IServiceRemotingClientFactory" /> when it is not specified.
    ///     </para>
    ///     <para>
    ///         Note that on client side
    ///         <see
    ///             cref="Client.ServiceProxy.Create{TServiceInterface}(Uri, Services.Client.ServicePartitionKey, Communication.Client.TargetReplicaSelector, string)" />
    ///         method create a default <see cref="V1.Client.ServiceProxyFactory" /> once and hence the provider lookup happens
    ...
    ///         <list type="number">
    ///             <item>
    ///                 In the entry <see cref="System.Reflection.Assembly" /> obtained by calling method
    ///                 <see cref="System.Reflection.Assembly.GetEntryAssembly" />
    ///             </item>
    ///             <item>
    ///                 In the <see cref="System.Reflection.Assembly" /> that defines the remote interface for which listener
    ///                 or proxy is being created.
    ///             </item>
    ///         </list>
    ///     </para>
    /// </remarks>
    ...
    /// <summary>
    ///     Determines the remoting stack for server/listener when using remoting provider attribuite to determine the remoting
    ///     client.
    /// </summary>
