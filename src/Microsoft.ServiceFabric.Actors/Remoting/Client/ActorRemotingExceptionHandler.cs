...
namespace Microsoft.ServiceFabric.Actors.Remoting.Client
{
    ...
    ///             <description>
    ///                 <para>
    ///                     Operations performed on the actor are retried from the client based on the exception handling
    ///                     logic.
    ///                     These exceptions represent various error condition including service failover. Therefore it is
    ///                     possible
    ...
    ///                     perspective duplicate operation will be performed by the clients and it should handle it in the
    ///                     similar
    ///                     manner as if the operation was already processed and then a duplicate message arrived.
    ///                 </para>
    ///                 <para>
    ///                     Exception related to duplicate operation being processed is handled by returning
    ...
    ///             <description>
    ///                 <para>
    ///                     Operations on the actors are performed using a turn based concurrency lock (
    ///                     <see cref="Microsoft.ServiceFabric.Actors.Runtime.ActorConcurrencySettings" />)
    ///                     that supports logical call context based reentrancy. In case of the long running actor operations
    ///                     it is
    ///                     possible for acquisition of this lock to time out. The acquisition of the lock can also time out in
    ///                     case of the deadlock
    ///                     situations (actor A and actor B calling each other almost at the same time).
    ///                 </para>
    ///                 <para>
    ///                     The exception related to concurrency lock timeout is handled by returning
    ///                     <see cref="ExceptionHandlingRetryResult" /> from the
    ///                     <see
    ///                         cref="IExceptionHandler.TryHandleException(ExceptionInformation, OperationRetrySettings, out ExceptionHandlingResult)" />
    ///                     method
    ...
    ///                 <para>
    ///                     The exception related to concurrency lock timeout is handled by returning
    ///                     <see cref="ExceptionHandlingThrowResult" /> from the
    ///                     <see
    ///                         cref="IExceptionHandler.TryHandleException(ExceptionInformation, OperationRetrySettings, out ExceptionHandlingResult)" />
    ///                     method,
    ///                     if the client performing the operation is another actor. In the deadlock situations this allows the
    ///                     call chain to unwind all the way
